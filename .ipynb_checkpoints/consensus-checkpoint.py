    ...
    for algo, metadata in ALGO_METADATA.items():
        ...
        try:
            with Dataset(infile, 'r') as ds:
                ...
                if algo == 'sic4dvar':
                    # existing sic4dvar block unchanged
                ...
                elif algo in ('hivdi', 'busboi'):                          # <-- ADD THIS BLOCK
                    swot_ts = datetime.datetime(2000, 1, 1, 0, 0, 0)
                    time_vals = algo_time.filled(np.nan) if hasattr(algo_time, 'filled') else np.array(algo_time, dtype=float)
                    algo_time = np.array([
                        (swot_ts + datetime.timedelta(seconds=float(t))).strftime("%Y-%m-%dT%H:%M:%SZ")
                        if not np.isnan(float(t)) else None
                        for t in time_vals
                    ])
